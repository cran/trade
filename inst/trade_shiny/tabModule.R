tabModuleUI <- function(id) {
  ...
  fluidPage(
      ...
      sidebarLayout(sidebarPanel(
        ...
        # Application title
        titlePanel(paste0("Simulate a ", id)),

        #htmlOutput(ns("urlText"),hr(),
        ...
        #),
        hr(),
        radioButtons(ns("calcElast"), "Calibrate model parameters using:",
        ...
        conditionalPanel(
          ...
          numericInput(ns("enterElast"), "Enter Market Elasticity:", value=-1,min=-Inf,max=0,step=.1#, width='75%'
          )
        ),hr(),

        radioButtons(ns("supply"), "Competitive Interaction:",
                     choices = c("Bertrand",
                                 # "2nd Score Auction",
                                 "Cournot"
                     )),

        selectInput(ns("demand"), "Demand Specification:",
                    choices = c("logit", "ces",
                                #"linear",
                                "aids")),
        hr(),
      ...
      mainPanel(
        ...
        #),
        actionButton(inputId =ns("simulate") , label = tags$b("Simulate"),style='padding:4px; font-size:125%')
        #)
        ,
        br(), br(),br(),
...
tabModule <- function(input, output, session){
  ...
  genInputData <- function(){
    ...
    colnames(inputData) <- gsub("Tariff|Quota",input$menu, colnames(inputData), perl = TRUE)
  ...
  gensum <- function(res, indata, tab=tolower(input$menu)){
    ...
    indata <- indata[!is.na(indata$Name),]
      ...
      tariffPre <- indata[,grepl("Cur.*(Tariff|Quota)",colnames(indata), perl = TRUE),drop=TRUE]
      tariffPre[is.na(tariffPre)] <- 0
      tariffPost <- indata[,grepl("New.*(Tariff|Quota",colnames(indata), perl=TRUE),drop=TRUE]
    ...
    if(tab == "tariff"){
      ...
      tariffPre <-  tariffPost <- rep(0, nrow(indata))
  ...
  runSims <- function(supply, demand, indata, mktElast,tab=tolower(input$menu)){
    # a function to execute code from antitrust package based on ui inputs

    prices <- indata[,"Prices \n($/unit)"]
    margins <- indata$Margins
    tariffPre <- indata[,grepl("Cur.*(Tariff|Quota)",colnames(indata)),drop=TRUE]
    tariffPre[is.na(tariffPre)] <- 0
    tariffPost <- indata[,grepl("New.*(Tariff|Quota)",colnames(indata)),drop=TRUE]
    tariffPost[is.na(tariffPost)] <- 0
  ...
  values <- reactiveValues(inputData = NULL, sim = NULL, msg = NULL)
  ...
  output$hot <- renderRHandsontable({
    ...
    if(tolower(input$menu)== "tariff"){colnames(inputData) <- gsub("Quota","Tariff", colnames(inputData)) }
    else{colnames(inputData) <- gsub("Tariff","Quota", colnames(inputData))}
  ...
  observeEvent(input$simulate,{
    ...
    tariffPost <- indata[,grep('New.*(Tariff|Quota)',colnames(indata)), drop = TRUE]
    tariffPost[is.na(tariffPost)] <- 0

    tariffPre <- indata[,grep('Cur.*(Tariff|Quota)',colnames(indata)), drop= TRUE]
    tariffPre[is.na(tariffPre)] <- 0


    if(tolower(input$menu)== "tariff"){
    indata$mcDelta <-  (tariffPost - tariffPre)
    indata$mcDelta <-  indata$mcDelta/(1 - tariffPost)
    }
    else{indata$mcDelta <- rep(0, nrow(indata))}
  ...
  output$results_detailed <- renderTable({
    ...
    if(input$supply == "Cournot"){
      ...
      if(tolower(input$menu)== "quota"){gsub("Tariff","Quota",colnames(res))}
    }
  ...
  output$results_elast <- renderTable({
    ...
    if(grepl("Current",input$pre_elast)){ preMerger = TRUE}
  ...
  output$results_mktelast <- renderTable({
    ...
    if(grepl("Current",input$pre_elast)){ preTariff = TRUE}
